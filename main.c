...
int main(void)
{
	...
	GPIOE->MODER |= 0x00080000;		// Set ouput mode of  pin 9 to alternate function mode
	...
	GPIOE->AFR[1] |= 0x0000020; //Setting PE.9 to output TIM1_CH1 is obtained using the following command
	...
	TIM1->CCMR1 |= 0x00000060; // The followed sets channel 1 to be standard PWM mode:
	TIM1->CCR1 = 10;		   //Sets on time to 10 clock pulses

	TIM1->BDTR |= TIM_BDTR_MOE;
	TIM1->CCER |= TIM_CCER_CC1E; //. The following enables output for Channel 1:
	...
	// // // GPIOE->MODER |= 0x00010000; // Set mode of each pin in port E
	// // GPIOE->MODER |= 0x55550000;		// Set ouput mode of  pin 8-15 so all are output mode
	// GPIOE->MODER |= 0x00080030;		// Set ouput mode of  pin 8-15 so all are output mode
	// GPIOE->OTYPER &= ~(0x00000100); // Set output type for each pin required in Port E(open drain for pin 8 and push pull for pin 12)
	// GPIOE->PUPDR &= ~(0x55550000);	// Set Pull up/Pull down resistor configuration for Port E(pin 8 and pin 12 both set to pull up resistor)
	// // 								// TIM3->CR1 &= ~CEN; // Disable TIM7 interrupt
	...
	// TIM1->PSC = 79;  // prescalor value in Timer ‘x’ as 100
	// TIM1->ARR = 9; // Auto-Reset Register of Timer ‘x’ set to 1000
	// TIM1->CCR1=4;
